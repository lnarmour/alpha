...
#[test]
fn explicit_external_signature_consumes_and_returns_linear_values() {
    let source = format!(
        "external move(linear) -> linear\n{}",
        LINEAR_TO_UNRESTRICTED
            .replace("outputs Y", "outputs linear Y")
            .replace("X[i]", "move(X[i])")
    );
...
#[test]
fn explicit_external_unrestricted_output_consumes_its_linear_argument() {
    let source = format!(
        "external observe(linear) -> unrestricted\n{}",
        LINEAR_TO_UNRESTRICTED
            .replace("outputs Y", "outputs linear Y")
            .replace("X[i]", "observe(X[i])")
    );
    assert!(diagnostics(&source).is_empty());
}

#[test]
fn system_call_ports_preserve_linear_values() {
    let source = r#"affine callee [N] -> {: N > 0}
    inputs linear X : {[i] : 0 <= i < N};
...
affine caller [N] -> {: N > 0}
    inputs linear A : {[i] : 0 <= i < N};
    outputs linear B : {[i] : 0 <= i < N};
    let (B) = callee[N](A);
.
"#;
...
#[test]
fn system_calls_support_multiple_linear_outputs() {
    let source = r#"affine split [N] -> {: N > 0}
    inputs linear X : {[i] : 0 <= i < N};
           linear Y : {[i] : 0 <= i < N};
    outputs linear A : {[i] : 0 <= i < N};
            linear B : {[i] : 0 <= i < N};
    let A[i] = X[i]; B[i] = Y[i];
.
...
#[test]
fn incomplete_linear_use_equation_output_is_diagnosed() {
    let source = r#"affine source [N] -> {: N > 0}
    inputs linear X : {[i] : 0 <= i < N};
    outputs linear Y : {[i] : 0 <= i < N};
    let Y[i] = X[i];
.
...
affine caller [N] -> {: N > 1}
    inputs linear X : {[i] : 0 <= i < N};
    outputs linear Y : {[i] : 0 <= i < N};
    let over {[i] : 0 <= i < N - 1} : (Y[i]) = source[N](X[i]);
.
"#;
    let diagnostics = diagnostics(source);
    ...
    assert!(
        diagnostics
            .iter()
            .any(|diagnostic| matches!(diagnostic, Diagnostic::LinearDefinitionIncomplete { .. })),
        "{diagnostics:#?}"
    );
}
