...
`prefix_scan.ipynb` is `docs/scheduled-codegen-design.md` §5.2's own worked example, executed for
...
```
pytest --nbval alpha-py/notebooks/prefix_scan.ipynb
```
...
all, so this never runs as part of the ordinary `alpha-py/tests/` suite. It needs the `python3`
...
diff on re-run means an actual behavior change in `alpha-py`/`alpha-codegen`, not fixture flakiness.

To regenerate after an intentional change, re-execute and overwrite in place:

```
jupyter nbconvert --to notebook --execute --inplace alpha-py/notebooks/prefix_scan.ipynb
```
