...
def ip():
    shell = get_ipython()
    shell.run_cell("import alphalang")
    # `alphalang.magics` self-registers via a module-level `get_ipython()` call, which is a no-op if
    # `alphalang` was already imported (by an earlier test module, e.g. test_alpha.py) before this
...
def test_percent_alpha_binds_a_system(ip):
    ...
    assert isinstance(ip.user_ns["sys"], alphalang.System)
...
def test_percent_schedule_binds_a_scheduled_system(ip):
    ...
    assert isinstance(ip.user_ns["sched"], alphalang.ScheduledSystem)
...
def test_percent_schedule_with_illegal_schedule_raises_schedule_error(ip):
    ...
    assert isinstance(result.error_in_exec, alphalang.ScheduleError)
