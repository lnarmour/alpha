...
| `alphalang.generate_makefile(c_path, wrapper_paths=[])` | `str` | issue #22, sub-issue of #21; a `Makefile` that compiles `c_path` to an object file and links each path in `wrapper_paths` against it into its own executable — takes file paths (not a `System`/`NormalizedSystem`/`ScheduledSystem`), since a Makefile has nothing to say about program semantics |
